...
fn search<NODE: NodeType>(
        ...
        && !tt_move.is_quiet()
    {
        ...
        while let Some(mv) = move_picker.next::<NODE>(td, true, ply) {
            ...
            let mut score = -search::<NonPV>(td, -probcut_beta, -probcut_beta + 1, probcut_depth, false, ply + 1);

            if score >= probcut_beta && probcut_depth > 0 {
                probcut_depth = (base_depth - (score - probcut_beta) / 319).clamp(0, base_depth);

                if probcut_depth < base_depth {
