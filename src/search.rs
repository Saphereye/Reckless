...
fn search<NODE: NodeType>(
            ...
            && td.board.piece_on(tt_move.to()).value() >= PieceType::Knight.value())
    {
        ...
        let prior_history = if td.stack[ply - 1].mv.is_quiet() {
            td.quiet_history.get(td.board.prior_threats(), !stm, td.stack[ply - 1].mv)
        } else {
            0
        };

        let r =
            (4311 - 200 + 1024 * improving as i32 + 260 * depth + 493 * (estimated_score - beta).clamp(0, 1003) / 128
                - prior_history / 8)
                / 1024;
